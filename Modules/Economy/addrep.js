    ...
    moment.locale('ru')
       ...
       if (member.id === message.author.id) return err('Вы не можете дать репутацию себе.');
        const embed = new Discord.RichEmbed()
            .setTitle(`Репутация <:rep:582206447245590578>`)
            .setColor('GREEN')
            .setDescription(`**${member.user.username}**, Вам дал(а) репутацию **${message.author.username}**!`)
          ...
          return err(`Вы уже давали репутацию, вы сможете дать её снова ${moment(cww).fromNow()}`)
        }
  }
  if(db.fetch(`langen_${message.guild.id}`)){
    ...
    let cww = repday.get(message.author.id);
       ...
       if (member.id === message.author.id) return err('You cannot give yourself a reputation.');
        const embed = new Discord.RichEmbed()
            .setTitle(`Reputation <:rep:582206447245590578>`)
            .setColor('GREEN')
            .setDescription(`**${member.user.username}**, Has given you a reputation **${message.author.username}**!`)
            .setFooter('JeggyBot, 2018-2019. ©', client.user.avatarURL)
            .setTimestamp();
            message.channel.send(embed);
           db.add(`rep_${message.guild.id}_${member.id}`, +1)
          ...
          return err(`You already gave a reputation, you can give it again through ${moment(cww).fromNow()}`)
        }
  }
    }
    module.exports.help = {
        name: ["rep", "like", "addrep"],
